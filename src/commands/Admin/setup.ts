...
@ApplyOptions<GirCommand.Options>({
  ...
  cooldownLimit: 2,
  description: `Easy and interactive setup for Gir`,
...
export class UserCommand extends GirCommand {
  public override async chatInputRun(
    ...
    collector.on('end', async (_c, r) => {
      ...
      if (r === 'Complete') {
        const data = await staffRolesSchema.findOneAndUpdate(
          { _id: interaction.guildId },
          { admin: adminRoles, moderator: modRoles }
        );
        const final = new Embed()
          ._color(GirColors.Success)
          ._title('Overview')
  ...
  private async step2(
    ...
    return i.update({
      ...
      components: [
        new Row<Button>()._components([
          new Button()
            ._customId('retry_admin')
            ._label('Retry')
            ._style(ButtonStyle.Secondary),
          new Button()
            ._customId('confirm_adminRoles')
            ._label('Confirm')
            ._style(ButtonStyle.Success),
        ]),
      ],
  ...
  private async step3(
    ...
    return i.update({
      ...
      components: [
        new Row<Button>()._components([
          new Button()
            ._customId('retry_modlog')
            ._label('Retry')
            ._style(ButtonStyle.Secondary),
          new Button()
            ._customId('make_modlog')
            ._label('Make a new modlog')
            ._style(ButtonStyle.Primary),
          new Button()
            ._customId('confirm_modlog')
            ._label('Confirm')
            ._style(ButtonStyle.Success),
        ]),
      ],
  ...
  private async step4(
    ...
    return i.update({
      ...
      components: [
        new Row<Button>()._components([
          new Button()
            ._customId('private_modlog')
            ._label('Private')
            ._style(ButtonStyle.Primary),
          new Button()
            ._customId('public_modlog')
            ._label('Public')
            ._style(ButtonStyle.Primary),
        ]),
      ],
