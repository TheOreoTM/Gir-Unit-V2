...
export class UserCommand extends GirCommand {
  ...
  public async messageRun(
    ...
    if (autoFilled) {
      ...
      for (const { author } of messages.values()) {
        if (author && !tributes.includes(author.username))
    ...
    const game: HungerGamesGame = Object.seal({
      ...
      llrc: new LongLivingReactionCollector(
        async (reaction) => {
          ...
          resolve(
            Boolean(
              this.kEmojis.indexOf(reaction.emoji.id ?? reaction.emoji.name!)
            )
          );
          resolve = null;
        },
      ...
      sun: true,
      tributes: this.shuffle(
        [...filtered].map(cleanMentions.bind(null, message.guild))
      ),
      turn: 0,
    });
  ...
  private async collectorInhibitor(
    message: GuildMessage,
    gameMessage: GuildMessage,
    reaction: LLRCData
  ) {
    ...
    if (!this.kEmojis.includes(reaction.emoji.id ?? reaction.emoji.name!))
      return true;
  ...
  private pick(
    events: readonly HungerGamesUsage[],
    tributes: number,
    maxDeaths: number
  ) {
    events = events.filter(
      (event) => event.tributes <= tributes && event.deaths.size <= maxDeaths
    );
  ...
  private calculateMaxDeaths(game: HungerGamesGame) {
    ...
    return game.tributes.size >= 16
      ...
      ? // If it's a bloodbath, perform mass death (12 -> 7), else eliminate 4.
        game.bloodbath
        ? Math.ceil(
            Math.min(game.tributes.size - 3, Math.sqrt(game.tributes.size) * 2)
          )
        : 4
