import {
  ...
  getQuestion,
  type QuestionData,
} from '#lib/games/TriviaManager';
import { GirCommand, LoadingEmbed } from '#lib/structures';
import {
  floatPromise,
  minutes,
  seconds,
...
export class triviaCommand extends GirCommand {
  #channels = new Set<string>();

  public async messageRun(message: Message, args: GirCommand.Args) {
    const category = await args
      .pick(triviaCommand.category)
      .catch(() => TrivaCategories.general);
    const questionType = await args
      .pick(triviaCommand.questionType)
      .catch(() => QuestionType.Multiple);
    const difficulty = await args
      .pick(triviaCommand.questionDifficulty)
      .catch(() => QuestionDifficulty.Easy);
    ...
    if (this.#channels.has(message.channel.id))
      this.error('A game of trivia is already being played in this channel');
    this.#channels.add(message.channel.id);

    try {
      await send(message, {
        embeds: [new LoadingEmbed(`Loading trivia`)],
      });
      ...
      const possibleAnswers =
        questionType === QuestionType.Boolean
          ? ['True', 'False']
          : shuffle(
              [data.correct_answer, ...data.incorrect_answers].map((ans) =>
                decode(ans)
              )
            );
      ...
      const filter = (msg: Message) => {
        ...
        return (
          Number.isInteger(num) && num > 0 && num <= possibleAnswers.length
        );
      };
      ...
      return collector
        .on('collect', (collected: Message) => {
          ...
          floatPromise(
            sendTemporaryMessage(
              collected,
              `I am sorry, but **${attempt}** is not the correct answer. Better luck next time!`
            )
          );
        })
        .on('end', () => {
  ...
  public buildQuestionEmbed(data: QuestionData, possibleAnswers: string[]) {
    const questionDisplay = possibleAnswers.map(
      (possible, i) => `${i + 1}. ${possible}`
    );
  ...
  private static questionType = Args.make<QuestionType>(
    (parameter, { argument }) => {
      const lowerCasedParameter = parameter.toLowerCase();
      if (
        lowerCasedParameter === 'boolean' ||
        lowerCasedParameter === 'truefalse'
      )
        return Args.ok(QuestionType.Boolean);
      if (lowerCasedParameter === 'multiple')
        return Args.ok(QuestionType.Multiple);
      return Args.error({ argument, parameter });
    }
  );

  private static questionDifficulty = Args.make<QuestionDifficulty>(
    (parameter, { argument }) => {
      const lowerCasedParameter = parameter.toLowerCase();
      if (lowerCasedParameter === 'easy')
        return Args.ok(QuestionDifficulty.Easy);
      if (lowerCasedParameter === 'medium')
        return Args.ok(QuestionDifficulty.Medium);
      if (lowerCasedParameter === 'hard')
        return Args.ok(QuestionDifficulty.Hard);
      return Args.error({ argument, parameter });
    }
  );
}
