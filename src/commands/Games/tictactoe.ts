...
import { promptConfirmation } from '#lib/utility/functions';
...
export class UserCommand extends GirCommand {
  ...
  public async messageRun(message: GuildMessage, args: GirCommand.Args) {
    if (this.channels.has(message.channel.id))
      this.error(
        `There is currently a game in progress in this channel, try again later.`
      );
  ...
  private getAuthorController(message: GuildMessage) {
    return new TicTacToeHumanController(
  ...
  private async getTargetController(message: GuildMessage, user: User) {
    if (user.bot)
      this.error(`You cant play games with a bot, they will never respond.`);
    if (user.id === message.author.id)
      this.error(`You cant play yourself, oh wait you already did.`);
