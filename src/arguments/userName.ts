...
import { isGuildMessage } from '#lib/utility';
import {
  SnowflakeRegex,
...
export class UserArgument extends Argument<User> {
  ...
  public async run(parameter: string, context: ArgumentContext<User>) {
    ...
    return this.error({
      parameter,
      identifier: `I couldn't find a user that resolved to ${parameter}`,
      context,
    });
  }
  ...
  private async resolveUser(message: GuildMessage, argument: string) {
    const result =
      UserOrMemberMentionRegex.exec(argument) ?? SnowflakeRegex.exec(argument);
