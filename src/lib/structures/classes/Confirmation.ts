...
export class Confirmation {
  ...
  public async run(
    ...
    const emojis = this.options?.emojis || [GirEmojis.Success, GirEmojis.Fail];
    const buttonLabels = this.options?.buttonLabels || ['Yes', 'No'];
    const row = new Row<ButtonBuilder>();
    const yes_button = new Button()
      ._customId('yes')
      ._label(buttonLabels[0])
      ._emoji(emojis[0])
      ._style(ButtonStyle.Success);

    const no_button = new Button()
      ._customId('no')
      ._label(buttonLabels[1])
      ._emoji(emojis[1])
      ._style(ButtonStyle.Danger);
    ...
    collector.on('collect', async (i) => {
      ...
      if (
        i.customId ===
        (this.buttons[0].data as APIButtonComponentWithCustomId).custom_id
      ) {
        await this.options.onConfirm({ i, msg });
      } else if (
        i.customId ===
        (this.buttons[1].data as APIButtonComponentWithCustomId).custom_id
      ) {
    ...
    collector.on('end', async (c) => {
      ...
      if (c.filter((b) => b.user.id === id)?.size === 0) {
        ...
        await msg.edit({
          ...
          embeds: this.options.content
            ? []
            : [embed.setDescription('No response received.')],
          components: [row],
        });
      }
    });
...
interface ConfirmationOptions {
  ...
  /**
   * Gir uses default emojis for confirmation, but custom can be added
   */
