...
GirEmojis.Backward;
export enum Emojis {
  Empty = GirEmojis.C4Empty,
  PlayerOne = GirEmojis.C4PlayerOne,
  PlayerTwo = GirEmojis.C4PlayerTwo,
  WinnerOne = GirEmojis.C4WinnerOne,
  WinnerTwo = GirEmojis.C4WinnerTwo,
}
...
export class ConnectFourGame extends BaseReactionGame<number> {
  ...
  protected renderOnEnd() {
    return this.winner === null
      ? `This game has concluded in a draw\n${this.renderBoard()}`
      : `${this.player.name} has won!\n${this.renderBoard()}`;
  }

  protected renderOnUpdateOrStart(): string {
    return `(${ConnectFourGame.players[this.turn]}) Turn for ${
      this.player.name
    }!\n${this.renderBoard()}`;
  }
  ...
  protected renderBoard(): string {
    ...
    for (let y = 0; y < rows; ++y) {
      ...
      for (let x = 0; x < columns; ++x) {
        line.push(ConnectFourGame.renderCell(this.getAt(x, y)).toString());
      }
