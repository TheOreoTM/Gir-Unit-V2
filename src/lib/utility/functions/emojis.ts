...
export function getEncodedTwemoji(emoji: string): EncodedTwemoji {
  return (
    TwemojiExceptions[emoji] ??
    [...emoji].map((point) => point.codePointAt(0)!.toString(16)).join('-')
  );
}
 ...
 * @param emoji The emoji to validate.
 */
export function isValidSerializedTwemoji(
  emoji: string
): emoji is SerializedEmoji {
 ...
 * @param emoji The emoji to validate.
 */
export function isValidSerializedCustomEmoji(
  emoji: string
): emoji is SerializedEmoji {
  return customEmojiRegExp.test(emoji);
}

export function isValidSerializedEmoji(
  emoji: string
): emoji is SerializedEmoji {
  return isSerializedTwemoji(emoji as SerializedEmoji)
    ? isValidSerializedTwemoji(emoji)
    : isValidSerializedCustomEmoji(emoji);
}
...
export function getEmojiTextFormat(emoji: SerializedEmoji): string {
  return isSerializedTwemoji(emoji)
    ? decodeURIComponent(emoji)
    : formatEmoji(emoji.slice(1), emoji.startsWith('a') as true | undefined);
}
...
export function getEmojiString(emoji: EmojiObject): SerializedEmoji {
  if (emoji.id)
    return `${emoji.animated ? 'a' : 's'}${emoji.id}` as SerializedEmoji;
...
export function resolveEmojiId(emoji: EmojiObject | SerializedEmoji): string {
  ...
  return typeof emoji === 'string'
    ? getEmojiId(emoji)
    : emoji.id ?? encodeURIComponent(emoji.name!);
}
