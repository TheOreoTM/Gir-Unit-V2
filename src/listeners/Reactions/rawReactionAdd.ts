...
import type { LLRCData } from '#lib/utility';
...
export class UserListener extends Listener {
  public run(raw: GatewayMessageReactionAddDispatch['d']) {
    const channel = this.container.client.channels.cache.get(raw.channel_id) as
      | TextChannel
      | undefined;
    if (!channel || !isGuildBasedChannel(channel) || !canReadMessages(channel))
      return;
    ...
    const data: LLRCData = {
      ...
      emoji: {
        ...
        user: (raw.emoji.user &&
          this.container.client.users['_add'](raw.emoji.user)) ?? {
          id: raw.user_id,
        },
      },
      guild: channel.guild,
    ...
    this.container.client.emit(GirEvents.RawReactionAdd, data, emoji);
  }
}
