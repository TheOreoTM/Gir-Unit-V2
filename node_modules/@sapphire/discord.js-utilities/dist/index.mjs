...
import { isNullish, partition, chunk, isFunction, isObject, deepClone } from '@sapphire/utilities';
import { ComponentType, ButtonStyle, PermissionsBitField, PermissionFlagsBits, ChannelType, Message, BaseInteraction, GuildMember, ActionRowBuilder, userMention, EmbedBuilder, Partials, IntentsBitField, GatewayIntentBits, ButtonBuilder, StringSelectMenuBuilder, InteractionCollector, InteractionType, isJSONEncodable } from 'discord.js';
...
function createPartitionedMessageRow(components) {
  ...
  const messageButtonActionRows = chunkedButtonComponents.map(
...
var _PaginatedMessage = class {
  ...
  async setUpMessage(messageOrInteraction, targetUser) {
    ...
    if (this.pages.length > 1) {
      const messageComponents = await Promise.all(
        [...this.actions.values()].map(async (interaction) => {
          return isMessageButtonInteractionData(interaction) ? new ButtonBuilder(interaction) : new StringSelectMenuBuilder({
  ...
  setUpCollector(channel, targetUser) {
    if (this.pages.length > 1) {
      this.collector = new InteractionCollector(targetUser.client, {
        filter: (interaction) => !isNullish(this.response) && //
        interaction.isMessageComponent() && this.actions.has(interaction.customId),
        time: this.idle,
  ...
  applyFooter(message, index) {
    ...
    if (lastEmbed) {
      const jsonTemplateEmbed = isJSONEncodable(this.template.embeds?.[idx] ?? this.template.embeds?.[0]) ? (this.template.embeds?.[idx] ?? this.template.embeds?.[0])?.toJSON() : this.template.embeds?.[idx] ?? this.template.embeds?.[0];
...
var PaginatedFieldMessageEmbed = class extends PaginatedMessage {
  constructor() {
    super(...arguments);
    __publicField(this, "embedTemplate", new EmbedBuilder());
