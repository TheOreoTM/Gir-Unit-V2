...
var ApplicationCommandRegistries_js = require('./ApplicationCommandRegistries.js');
var computeDifferences_js = require('./computeDifferences.js');
var normalizeInputs_js = require('./normalizeInputs.js');
...
var ApplicationCommandRegistry = class {
  ...
  get command() {
    return pieces.container.stores.get("commands").get(this.commandName);
  }
  registerChatInputCommand(command, options) {
    const builtData = normalizeInputs_js.normalizeChatInputCommand(command);
    this.chatInputCommands.add(builtData.name);
    this.apiCalls.push({
      builtData,
      registerOptions: options ?? { registerCommandIfMissing: true, behaviorWhenNotIdentical: ApplicationCommandRegistries_js.getDefaultBehaviorWhenNotIdentical() },
      type: Enums_js.InternalRegistryAPIType.ChatInput
    });
    if (options?.idHints) {
    ...
    if (!utilities.isNullishOrEmpty(options?.guildIds)) {
      for (const id of options.guildIds) {
        this.guildIdsToFetch.add(id);
        ApplicationCommandRegistries_js.allGuildIdsToFetchCommandsFor.add(id);
      }
    }
    return this;
  }
  registerContextMenuCommand(command, options) {
    const builtData = normalizeInputs_js.normalizeContextMenuCommand(command);
    this.contextMenuCommands.add(builtData.name);
    this.apiCalls.push({
      builtData,
      registerOptions: options ?? { registerCommandIfMissing: true, behaviorWhenNotIdentical: ApplicationCommandRegistries_js.getDefaultBehaviorWhenNotIdentical() },
      type: Enums_js.InternalRegistryAPIType.ContextMenu
    });
    if (options?.idHints) {
      for (const hint of options.idHints) {
  ...
  async handleAPICall(commandsManager, globalCommands, allGuildsCommands, apiCall) {
    ...
    const behaviorIfNotEqual = registerOptions.behaviorWhenNotIdentical ?? ApplicationCommandRegistries_js.getDefaultBehaviorWhenNotIdentical();
    const findCallback = /* @__PURE__ */ __name((entry) => {
      if (apiCall.type === Enums_js.InternalRegistryAPIType.ChatInput && entry.type !== v10.ApplicationCommandType.ChatInput)
        return false;
      if (apiCall.type === Enums_js.InternalRegistryAPIType.ContextMenu) {
        if (entry.type === v10.ApplicationCommandType.ChatInput)
          return false;
    ...
    let type;
    switch (apiCall.type) {
      case Enums_js.InternalRegistryAPIType.ChatInput:
        type = "chat input";
        break;
      case Enums_js.InternalRegistryAPIType.ContextMenu:
        switch (apiCall.builtData.type) {
          case v10.ApplicationCommandType.Message:
            type = "message context menu";
            break;
          case v10.ApplicationCommandType.User:
  ...
  async handleCommandPresent(applicationCommand, apiData, behaviorIfNotEqual, guildId) {
    ...
    if (behaviorIfNotEqual === Enums_js.RegisterBehavior.BulkOverwrite) {
      this.debug(
      ...
      behaviorIfNotEqual = ApplicationCommandRegistries_js.getDefaultBehaviorWhenNotIdentical();
      if (behaviorIfNotEqual === Enums_js.RegisterBehavior.BulkOverwrite) {
        throw new Error(
    ...
    let differences = [];
    if (behaviorIfNotEqual === Enums_js.RegisterBehavior.VerboseOverwrite) {
      const now = Date.now();
      differences = [...computeDifferences_js.getCommandDifferences(normalizeInputs_js.convertApplicationCommandToApiData(applicationCommand), apiData, guildId !== null)];
    ...
    if (behaviorIfNotEqual === Enums_js.RegisterBehavior.Overwrite || behaviorIfNotEqual === Enums_js.RegisterBehavior.LogToConsole) {
      const now = Date.now();
      const areThereDifferences = computeDifferences_js.getCommandDifferencesFast(normalizeInputs_js.convertApplicationCommandToApiData(applicationCommand), apiData, guildId !== null);
      const later = Date.now() - now;
    ...
    this.logCommandDifferencesFound(applicationCommand, behaviorIfNotEqual === Enums_js.RegisterBehavior.LogToConsole, differences);
    if (behaviorIfNotEqual === Enums_js.RegisterBehavior.LogToConsole) {
      return;
    }
    try {
  ...
  async createMissingCommand(commandsManager, apiData, type, guildId) {
    try {
      ...
      switch (apiData.type) {
        case void 0:
        case v10.ApplicationCommandType.ChatInput:
  ...
  info(message, ...other) {
    pieces.container.logger.info(`ApplicationCommandRegistry[${this.commandName}] ${message}`, ...other);
  }
  error(message, ...other) {
    pieces.container.logger.error(`ApplicationCommandRegistry[${this.commandName}] ${message}`, ...other);
  }
  warn(message, ...other) {
    pieces.container.logger.warn(`ApplicationCommandRegistry[${this.commandName}] ${message}`, ...other);
  }
  debug(message, ...other) {
    pieces.container.logger.debug(`ApplicationCommandRegistry[${this.commandName}] ${message}`, ...other);
  }
  trace(message, ...other) {
    pieces.container.logger.trace(`ApplicationCommandRegistry[${this.commandName}] ${message}`, ...other);
  }
};
__name(ApplicationCommandRegistry, "ApplicationCommandRegistry");
