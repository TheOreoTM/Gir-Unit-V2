...
var CoreListener = class extends Listener_js.Listener {
  constructor(context) {
    super(context, { event: Events_js.Events.PreMessageParsed });
  ...
  async run(message) {
    ...
    if (mentionPrefix) {
      if (message.content.length === mentionPrefix.length) {
        client.emit(Events_js.Events.MentionPrefixOnly, message);
        return;
      }
    ...
    if (prefix === null)
      client.emit(Events_js.Events.NonPrefixedMessage, message);
    else
      client.emit(Events_js.Events.PrefixedMessage, message, prefix);
  }
...
//# sourceMappingURL=CorePreMessageParser.js.map